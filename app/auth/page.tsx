    ...
    <main className="min-h-screen">
      <section className="hero-section relative flex min-h-screen items-center justify-center">
        ...
        <div className="container relative z-10 text-center">
          ...
          <div
            className="mx-auto mt-4 max-w-[350px]"
            style={{
          ...
          <div className="contact-section">
            ...
            {/* Sign-in for approved users */}
            <div className="mt-6">
              <p className="text-white/90 mb-3">{"Already have access?"}</p>
              <SignInButton mode="modal">
                <button className="contact-button" aria-label="Sign in">
                  {"Sign in"}
                </button>
              </SignInButton>
            </div>
      ...
      {/* Page-specific styles to match your provided design */}
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif; line-height: 1.6; color: #333; overflow-x: hidden; }
        ...
        .floating-elements { position: absolute; inset: 0; overflow: hidden; z-index: 1; }
        ...
        .main-message { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); padding: 3rem; border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); margin: 2rem auto; max-width: 600px; animation: slideInFromBottom 1s ease-out 0.6s both; border: 1px solid rgba(255,255,255,0.2); }
        .thank-you { font-size: 2.5rem; font-weight: 700; margin-bottom: 1rem; background: linear-gradient(45deg, #2e7d32, #4caf50); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .message-text { font-size: 1.2rem; color: #555; line-height: 1.8; margin-bottom: 2rem; }

        .features-preview { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 2rem; margin: 3rem 0; animation: fadeInUp 1s ease-out 0.9s both; }
        .feature-card { background: rgba(255,255,255,0.1); padding: 2rem; border-radius: 15px; backdrop-filter: blur(5px); border: 1px solid rgba(255,255,255,0.2); transition: all 0.3s ease; cursor: pointer; }
        ...
        .contact-section { margin-top: 4rem; animation: fadeInUp 1s ease-out 1.2s both; }
        .contact-title { font-size: 2rem; color: white; margin-bottom: 1rem; font-weight: 600; }
        .contact-text { color: rgba(255,255,255,0.9); font-size: 1.1rem; margin-bottom: 2rem; }
        .contact-button { background: linear-gradient(135deg, #4caf50, #45a049); color: white; padding: 15px 30px; border: none; border-radius: 50px; font-size: 1.1rem; font-weight: 600; cursor: pointer; transition: all 0.3s ease; text-decoration: none; display: inline-block; box-shadow: 0 4px 15px rgba(76,175,80,0.3); }
        ...
        .scroll-indicator { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); color: rgba(255,255,255,0.8); animation: bounce 2s infinite; }
      ...
      {/* Minimal interactivity to mimic your original script */}
      <script
